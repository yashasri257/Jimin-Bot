...
async def view(inter, card_code:str):
    ...
    print("VIEW COMMAND TRIGGERED")
    print("Image URL:", card[5])
...
class DropView(disnake.ui.View):
    ...
    async def b1(self, button, inter): await self.claim(inter,0)

    @disnake.ui.button(label="2")
    async def b2(self, button, inter): await self.claim(inter,1)

    @disnake.ui.button(label="3")
    async def b3(self, button, inter): await self.claim(inter,2)
